...
assert.match(indexHtml, /发现并接入由本机 Agent 生成的项目产物，确认前不会导入任何文件/, "Artifact Access must use concise product copy without exposing provider-specific discovery mechanics");
...
assert.match(indexHtml, /\.nbskill-enhancement-hint:not\(\[hidden\]\)[\s\S]*?animation:\s*nbskill-enhancement-pulse[\s\S]*?@keyframes nbskill-enhancement-pulse[\s\S]*?@media \(prefers-reduced-motion: reduce\)[\s\S]*?\.nbskill-enhancement-hint:not\(\[hidden\]\)[\s\S]*?animation:\s*none/, "the first-install nbskill warning must pulse gently and respect reduced-motion preferences");
...
assert.match(indexHtml, /function agentArtifactSkillName[\s\S]*?nbskill_registered[\s\S]*?skillDisplayName[\s\S]*?source-kind-badge[\s\S]*?escapeHtml\(skillName\)/, "registered artifact rows must show the real producing Skill recorded in manifest evidence");
...
assert.match(indexHtml, /const indexedPaths = \[\.\.\.new Set\(\[[\s\S]*?group\.alreadyIndexedPaths[\s\S]*?indexedArtifacts\.map[\s\S]*?indexedRows/, "expanded project details must list connected artifact paths even when no review candidates remain");
assert.match(indexHtml, /preview\.indexedArtifacts[\s\S]*?indexedSkillByPath[\s\S]*?agentArtifactSkillName\(candidate\)[\s\S]*?indexedArtifacts\.map[\s\S]*?source-kind-badge/, "indexed manifest artifacts must remain visible with their producing Skill after auto-import");
assert.match(indexHtml, /preview\.project\?\.manifestPath[\s\S]*?content\.manifestStatus[\s\S]*?lastManifestOkAt/, "hybrid project details must expose manifest version and last successful read time");
...
assert.match(indexHtml, /id="settingsNbskillEnhancementHint"[\s\S]*?settings\.nbskillEnhancementHint[\s\S]*?hasInstalledNbskill = statuses\.some\(\(status\) => Boolean\(status\.installedVersion\)\)[\s\S]*?\.hidden = hasInstalledNbskill/, "the nbskill enhancement hint must remain visible until at least one Agent has an installed package");
assert.match(indexHtml, /function loadNbskillAgentStatuses[\s\S]*?get_nbskill_agent_status[\s\S]*?length !== 5[\s\S]*?loadFailed/, "nbskill detection must reject incomplete status responses while retaining last-known state");
...
assert.match(indexHtml, /function copySelectedNbskillInstallPrompt[\s\S]*?selectedNbskillAgentId[\s\S]*?prepare_nbskill_install_prompt[\s\S]*?agentId: selected\.agentId[\s\S]*?response\.copied/, "switching Agent must keep the host-generated install prompt target and copy result aligned");
assert.doesNotMatch(indexHtml, /copySelectedNbskillInstallPrompt[\s\S]{0,1200}navigator\.clipboard/, "nbskill copy must not depend on WebView clipboard permission");
assert.match(indexHtml, /response\.copied[\s\S]*?window\.prompt\(currentNbskillStatusContent\(\)\.clipboardFallback, response\.prompt\)/, "host clipboard failure must still expose the complete generated prompt for manual recovery");
assert.match(nbskillCommandsRust, /NSPasteboard::generalPasteboard\(\)[\s\S]*?clearContents\(\)[\s\S]*?setString_forType[\s\S]*?stringForType[\s\S]*?copied\.to_string\(\) == text/, "macOS packaged copy must write and read back the native AppKit pasteboard before reporting success");
...
assert.match(mainRust, /commands::nbskill::get_nbskill_agent_status[\s\S]*?commands::nbskill::prepare_nbskill_install_prompt/, "both read-only status and user-mediated install-prompt commands must be registered with Tauri");
...
assert.match(connectAgentScopes, /if \(acceptSuggestedImmediately && batchKeys\.length\)[\s\S]*?}\s*await loadLibraries\(\);[\s\S]*?await loadItems\(\);[\s\S]*?agentDiscoveryPayload = await invoke\("discover_agent_projects"\)/, "opening details must refresh indexed paths after manifest auto-import even when no suggested batch remains");

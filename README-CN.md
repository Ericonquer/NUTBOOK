...
接下来会继续沿着“让 AI 产物更容易阅读、展示、沉淀和复用”这个方向往前推：

* **Markdown 导出中心**：先支持 Markdown 导出为阅读型 HTML 和展示型 HTML，再逐步接入 PDF、长图和水印。

* **HTML 转 Markdown**：把 HTML 中的文字和结构提取出来，转换成可继续整理的 Markdown。

* **AI 辅助产物修订**：由前置 AI 工作流负责结构与布局调整，NUTBOOK 聚焦确定性的本地审阅和轻量修正。

* **画笔工具**：在 Markdown 和 HTML 展示时提供圈划能力，服务会议讲解、课堂演示和提案场景。

* **第三种文件视图**：提供更纯粹的文件名列表，并在右侧显示缩略图预览，提升大批量文件浏览效率。

* **基础体验完善**：继续优化标签、来源分类、自动扫描、路径映射和内容生成型 skill 适配。

var NutbookDocumentTitle = (() => {
  ...
  // src/markdown-document-title.js
  var markdown_document_title_exports = {};
  __export(markdown_document_title_exports, {
    headingDisplayText: () => headingDisplayText,
    inlineDisplayText: () => inlineDisplayText,
    parseDocumentTitle: () => parseDocumentTitle,
    setDocumentTitleInSource: () => setDocumentTitleInSource
  });
  ...
  // src/markdown-document-title.js
  var AST_EXTENSIONS = [gfmStrikethrough()];
  var MDAST_EXTENSIONS = [gfmStrikethroughFromMarkdown()];
  function parseDocumentTitle(markdown, fallbackTitle = "Markdown") {
    ...
    for (let index2 = 0; index2 < children.length; index2 += 1) {
      ...
      if (node2.type !== "heading" || node2.depth !== 1) continue;
      const display = headingDisplayText(node2.children);
      if (!display) continue;
      const prev = children[index2 - 1];
      const next = children[index2 + 1];
      if (prev && next && prev.type === "html" && isAlignedOpen(prev.value) && next.type === "html" && isAlignedClose(next.value)) {
        return {
          displayText: display,
          source: "aligned-h1",
          ...
          locator: {
            ...
            titleLine: mdastLine(node2.position, frontmatterLines)
          }
        };
      }
      const startLine = mdastLine(node2.position, frontmatterLines);
      const endLine = mdastEndLine(node2.position, frontmatterLines);
  ...
  function setDocumentTitleInSource(markdown, nextTitle, options = {}) {
  ...
  function inlineDisplayText(input) {
  ...
  function headingDisplayText(children) {
    let out = "";
  ...
  function mdastLine(position2, frontmatterLines) {
    const line = position2 && position2.start ? position2.start.line : 1;
    return line - 1 + frontmatterLines;
  }
  function mdastEndLine(position2, frontmatterLines) {
    const line = position2 && position2.end ? position2.end.line : 1;
    return line - 1 + frontmatterLines;
  }
  ...
  return __toCommonJS(markdown_document_title_exports);
})();

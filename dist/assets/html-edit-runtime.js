(function () {
  ...
  const STATE = {
    ...
    inlineToolbar: null, inlineToolbarEnabled: false, inlineToolbarDock: "bottom", inlineToolbarDrag: null, locale: "", imageFrameLabels: null, saveNotice: "",
  ...
  async function enter(payload) {
    ...
    STATE.imageFrameLabels = normalizeImageFrameLabels(payload.imageFrameLabels || payload.labels);
  ...
  function normalizeImageFrameLabels(labels) {
    if (!labels || typeof labels !== "object") return null;
    const confirm = String(labels.confirm || labels.confirmImageFrame || "").trim();
    const cancel = String(labels.cancel || labels.cancelImageFrame || "").trim();
    return confirm || cancel ? { confirm, cancel } : null;
  }
  function imageFrameLabel(kind) {
    const provided = STATE.imageFrameLabels?.[kind];
    if (provided) return provided;
    const english = String(STATE.locale || "").toLowerCase().startsWith("en");
    return english
      ? ({ confirm: "Confirm frame", cancel: "Cancel frame" }[kind] || kind)
      : ({ confirm: "确认框选", cancel: "取消框选" }[kind] || kind);
  }
  function updateLocale(payload = {}) {
    const options = typeof payload === "string" ? { locale: payload } : (payload || {});
    if (options.runtimeSessionId && options.runtimeSessionId !== STATE.sessionId) return false;
    if (options.locale) STATE.locale = String(options.locale);
    if (Object.prototype.hasOwnProperty.call(options, "imageFrameLabels") || Object.prototype.hasOwnProperty.call(options, "labels")) {
      STATE.imageFrameLabels = normalizeImageFrameLabels(options.imageFrameLabels || options.labels);
    }
    if (STATE.editing) layoutInsertedImages();
    return true;
  }
  ...
  window.__NUTBOOK_HTML_EDIT__ = { scanEditableElements, isEditing: () => STATE.editing, enter, updateLocale, exit, markSaved, rebaseSaved: markSaved, collectChanges, applyPatch, applyFormat, applyImportedAsset, beginInsertedImageDraft, commitInsertedImageDraft, cancelInsertedImageDraft, applyInsertedImageAsset, requestInsertedImageReplacement, resumeInsertedImageDraft, resumeInsertedImageReplacement, beginInsertedImageCrop, deleteInsertedImage, undoHistory, redoHistory, goToPresentationPage, goToAdjacentPresentationPage, presentationSnapshot, goToSection, refreshSectionNavigation, sectionNavigationSnapshot, getSnapshot, reportState, emitHostMessage };
})();

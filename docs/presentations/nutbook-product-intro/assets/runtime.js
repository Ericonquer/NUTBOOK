...
(function(){
  ...
  timerCount.textContent = (idx + 1) + ' / ' + total;
    ...
    function cycleAnim(){
      ...
      const target = slides[idx].querySelector('[data-anim-target]') || slides[idx].querySelector('.h1, .h2, .cover-copy, .grid, .flow, .audience-layout, .library-grid, .reading-demo, .stage, .ingest-board, .timeline, .promise-grid, .closing-line');
      if (!target) return;
